...
# tauz=seq(0,24,length.out=100)
# plot(tauz,pnorm(tauz,psi_hat[3],psi_hat[4])-pnorm(tauz,psi_hat[1],psi_hat[2]),type="l");abline(h=0,v=13.53)
# plot(tauz,1-pnorm(tauz,psi_hat[3],psi_hat[4])-pnorm(tauz,psi_hat[1],psi_hat[2]),type="l");abline(h=0,v=13.53)
...
# Compass Search (faster than brute force.  Consider using!)
...
# admissable = function(x) 0<x[2] & 0<x[4] & 0<x[6] & 0<x[8]
...
#   output = scores(x, zs, taus, diff_taus)
#   objective_function = sum(output**2)
#   return(list(obj=objective_function,anc=list(psi_hat=x)))
# }
