plot_data_quality_predictiveness = function(filters, ...){
  ...
  for(filter in 1:length(names(filters))){
    ...
    plot(0,0,col=NA,yaxt="n",xaxt="n",xlab="Weeks Ahead",ylab="Data Quality Metric",xlim=c(1.04,7-.04),ylim=c(1.04,7-.04),axes=F,main=name_filter)
    axis(1,at=1:6+.5, labels = 1:6-1)
    axis(2,at=1:6+.5, labels = colnames(coefs))
    for(i in 1:6){
      for(j in 1:6){
        C=1-min(abs(coefs[i,j]),3)/3
        if(coefs[i,j] > 0)  polygon(i+c(0,0,1,1), j+c(0,1,1,0),col=rgb(C,C,1),pch=15,cex=2,border=FALSE)
        if(coefs[i,j] <= 0) polygon(i+c(0,0,1,1), j+c(0,1,1,0),col=rgb(1,C,C),pch=15,cex=2,border=FALSE)
      }
    }
    for(i in 1:6){
      for(j in 1:6){
        if(abs(coefs[i,j]) > -log10(0.05)) polygon(i+c(0,0,1,1), j+c(0,1,1,0),col=NA,lwd=2)
      }
    }
  }
  dev.off()
