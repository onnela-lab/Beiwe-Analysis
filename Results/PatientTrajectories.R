PatientTrajectoriesPlots = function(pltname,y,X,dat,surveylab=NULL,MMH=1439,MML=1440*5/6){
  for(i in 1:nrow(dat)){
...
patient_trajectories_plots = function(inds=NULL,pltname=NULL,labels=NULL,...){
  featurefile=paste(output_filepath,"Processed_data","Group","feature_matrix_clean.rds",sep="/")
  MinsMissingCutLOW=1200
  MinsMissingCutHIGH=1440
  if(!file.exists(featurefile)){return(NULL)}
  inlist=readRDS(featurefile)
  dat=inlist[[1]]
  ...
  minx3=min(x3,na.rm=T);maxx3=max(x3,na.rm=T)
  if(is.null(pltname)){
    pltname = paste("patient_trajectories_", gsub("/","-",max(dat[,2])),".pdf",sep="")
  }
  y=c(1,10)
  pdf(paste(output_filepath,"Results","Group",pltname,sep="/"),width=6,height=3)
  plot(NA,xlim=c(0,1),ylim=c(0,1),xaxt="n",yaxt="n",xlab="",ylab="",bty="n",main="Patient trajectories")
