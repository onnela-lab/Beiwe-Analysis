...
CreateMobilityFeatures = function(patient_id, ...){
  pcsfilename = paste(output_filepath, "Processed_data", "Individual", patient_id, "gps_imputed_mobmat.rds", sep="/")
  if(!file.exists(pcsfilename)){
    cat("No imputed data.\n")
    return(NULL)
  }  
  loadRDS(pcsfilename)
  prepcsfilename = paste(output_filepath, "Preprocessed_data", "Individual", patient_id, paste("gps_preprocessed",patient_id,".rds",sep=""),sep="/")
  if(!file.exists(prepcsfilename)){
    cat("No preprocessed data.\n")
  }else{
    pre_data = readRDS(prepcsfilename)
    mobmatsims = pre_data$mobmatsims
    objsims = pre_data$objsims
    nreps = pre_data$nreps
    ...
    outfilename = file.path(paste(output_filepath, "Processed_data", "Individual", patient_id, sep="/"),paste("MobFeatures_", patient_id, ".rds", sep=""))
    saveRDS(list(outmat = outmat, lsmf = lsmf, lssigloc = lssigloc, featavg = featavg), outfilename)
  }
}
...
daily_features = function(...){
  ...
  SIDs = lapply(strsplit(list.dirs(data_filepath, recursive = FALSE),"/"),function(x) x[length(x)])
  qIDs = list()
  aIDs_full = list()
  mob_full = list()
  for(i in 1:length(SIDs)){
    ...
    mobmat=NULL
    aIDs=list()
    mobfilename = file.path(paste(output_filepath, "Processed_data", "Individual", patient_id, sep="/"), paste("MobFeatures_", patient_id, ".rds", sep=""))
    ...
    # Survey Answers
    if(file.exists(paste(data_filepath, patient_id, "survey_answers", sep="/"))){
  ...
  outmat = data.frame(outmat,stringsAsFactors = FALSE)
  ...
  for(i in 1:length(SIDs)){
    setwd(paste(datadir,SIDs[[i]],sep="/"))
    if(file.exists(paste(datadir,SIDs[[i]],"texts", sep="/"))){
      textmat = c()
      text_files = list.files(paste(datadir,SIDs[[i]],"texts", sep="/"))
      for(text_file in text_files)
        textmat = rbind(textmat, data = read.csv(paste(datadir, "/", SIDs[[i]], "/texts/", text_file, sep=""), header = T))
    ...
    if(file.exists(paste(datadir, SIDs[[i]], "calls", sep="/"))){
      callmat = c()
      call_files = list.files(paste(datadir,SIDs[[i]], "calls", sep="/"))
      for(call_file in call_files)
        callmat = rbind(callmat, data = read.csv(paste(datadir, "/", SIDs[[i]], "/calls/", call_file, sep=""), header = T))
  ...
  write.table(outmat, file = paste(output_filepath, "Group", "Processed_data", "FeatureMatrix.txt", sep = "/"), sep = "\t", row.names = FALSE, col.names = TRUE)
}
...
DailyRoutineIndex = function(indday, mobmat, subsetinds_v, subsetdayofweek_v, subsetstarttime_v, tz, CENTERRAD){
  ...
  if(subsetdayofweek_v[indday] == "Saturday" || subsetdayofweek_v[indday] == "Sunday"){
    ...
    IDcompare = c(which(subsetdayofweek_v == "Monday"),   which(subsetdayofweek_v == "Tuesday"), which(subsetdayofweek_v == "Wednesday"),
	              which(subsetdayofweek_v == "Thursday"), which(subsetdayofweek_v == "Friday"))
  }
